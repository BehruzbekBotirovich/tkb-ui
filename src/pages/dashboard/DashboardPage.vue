...
<template>
    ...
    <a-row>
        <a-col :lg="6" class="stat-block">
            <StatisticsTable :title="`${t('statisticsTable.region')}`" :hasLink="true"
                :description="`${t('statisticsTable.regionName')}`" :data="store.regionalTable"> </StatisticsTable>
        </a-col>
        <a-col :lg="6" class="stat-block">
            <StatisticsTable :title="`${t('statisticsTable.univer')}`" :hasLink="false"
                :description="`${t('statisticsTable.regionName')}`" :data="store.universityTable">
            </StatisticsTable>
        </a-col>
        <a-col :lg="6" class="stat-block">
            <StatisticsTable :title="`${t('statisticsTable.banks')}`" :hasLink="false"
                :description="`${t('statisticsTable.banksName')}`" :data="store.bankTable">
            </StatisticsTable>
        </a-col>
        <a-col :lg="6" class="stat-block">
            <GenderDiagram></GenderDiagram>
        </a-col>
    </a-row>
</template>
<style scoped>
